...
const MainPage = () => {
    ...
    const fetchJobRecommendations = async () => {
      try {
        const response = await fetch(
          "http://localhost:8000/user_recomdation_jobs",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        const data = await response.json();
        setPreferJobTitle(data.prefer_job_title);
        setPredictedTitle(data.predicted_title);
      } catch (error) {
  ...
  const filteredJobs = jobs.filter((job) => {
    const jobTitleLower = job.title.toLowerCase();
    return (
      (jobTitleLower.includes(preferJobTitle.toLowerCase()) ||
        jobTitleLower.includes(predictedTitle.toLowerCase())) &&
      (job.company.toLowerCase().includes(searchTerm.toLowerCase()) ||
        job.location.toLowerCase().includes(searchTerm.toLowerCase()))
    );
  });
  ...
  return (
    <div>
      ...
      <div
        className={`fixed bottom-0 right-0 mb-4 mr-4 ${isChatboxOpen ? "hidden" : ""
          }`}
      >
        <button
